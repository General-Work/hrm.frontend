...
<div class:text-sm={textSmall} class="flex gap-2 justify-between w-full items-center text-gray-600">
	...
	<!-- <div class:hidden={!tableColumns.length} class:flex={tableColumns}>
		...
		<Dropdown
			class="overflow-y-auto h-56 px-3 pb-3 text-sm scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch"
		>
			{#each tableColumns as column}
				<li class="flex p-2 hover:bg-gray-100 dark:hover:bg-gray-600">
					<Toggle bind:checked={hideState[column.id]} />
					{column.header}
				</li>
			{/each}
		</Dropdown>
