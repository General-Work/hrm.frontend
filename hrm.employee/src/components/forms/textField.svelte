...
<fieldset class="flex flex-col gap-1.5 {otherClasses}">
	...
	{:else}
		<input
			...
			class:readonly
			class="border border-gray-300 shadow-gray-50 rounded-[5px] shadow text-black w-full h-[40px] outline-transparent focus:outline-transparent focus:outline-none px-3 placeholder:text-gray-500/80 dark:shadow-transparent"
			{placeholder}
			{readonly}
			{id}
			value={$data[name] || ''}
